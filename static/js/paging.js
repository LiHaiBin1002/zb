(function($, window, document) {
    ...
    function Paging(el, options) {
        this.el = el;
        this.options = {
            pageSelect: options.pageSelect || 'pageSelect',
            prePage: options.prePage || 'prePage',
            nextPage: options.nextPage || 'nextPage',
            firstPage: options.firstPage || 'firstPage',
            lastPage: options.lastPage || 'lastPage',
            jumpBtn: options.jumpBtn || 'jumpBtn',
            jumpText: options.jumpText || 'jumpText',
    ...
    Paging.prototype = {
        ...
        createDom: function() {
            ...
            that.options.jump ? jumpDom = '<input type="text" placeholder="1" class="jump-text" id="'+ that.options.jumpText +'"><button type="button" class="jump-button" id="'+ that.options.jumpBtn +'">跳转</button>' : jumpDom = '';
            content = '<button type="button" id="'+ that.options.firstPage +'" class="turnPage first-page">首页</button>' +
                '<button class="turnPage" id="'+ that.options.prePage +'">上一页</button>' +
                '<div class="pageWrap" style="width:' + wrapLength + 'px">' +
                '<ul id="'+ that.options.pageSelect +'" style="transition:all ' + that.options.slideSpeed + 'ms">' +
                ulDom +
                '</ul></div>' +
                '<button class="turnPage" id="'+ that.options.nextPage +'">下一页</button>' +
                '<button type="button" id="'+ that.options.lastPage +'" class="last-page">尾页</button>' +
                jumpDom +
        ...
        bindEvents: function() {
            var that = this,
                pageSelect = $('#' + that.options.pageSelect), // ul
                lis = pageSelect.children(), // li的集合
                liWidth = lis[0].offsetWidth, // li的宽度
                totalPages = that.options.totalPages, // 总页数
                pageIndex = that.options.pageNo, // 当前选择的页码
                distance = 0, // ul移动距离
                prePage = $('#' + that.options.prePage),
                nextPage = $('#' + that.options.nextPage),
                firstPage = $('#' + that.options.firstPage),
                lastPage = $('#' + that.options.lastPage),
                jumpBtn = $('#' + that.options.jumpBtn),
                jumpText = $('#' + that.options.jumpText);
